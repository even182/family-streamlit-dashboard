...
# - Secrets（可選）：
#   GOOGLE_DRIVE_XLSX_URL = "https://docs.google.com/spreadsheets/d/<ID>/edit?... 或 export 直連"
# - 若未設定 Secrets，會使用程式內預設的 URL（你提供的連結）
# =========================

DEFAULT_GDRIVE_URL = "https://docs.google.com/spreadsheets/d/1DHD3NdHF2QFgxvIAwhL74RUTqEWm_4zC/edit?usp=sharing"

def _to_gsheets_export_xlsx_url(url: str) -> str:
    """把 Google Sheets 連結轉成可直接下載的 xlsx 連結。"""
    ...
    # 已經是 export 直連就直接回傳
    if "docs.google.com/spreadsheets" in u and "/export" in u and "format=xlsx" in u:
        return u

    # 從 /spreadsheets/d/<ID>/... 抽出 ID
    m = re.search(r"/spreadsheets/d/([^/]+)/", u)
    if m:
        sheet_id = m.group(1)
        return f"https://docs.google.com/spreadsheets/d/{sheet_id}/export?format=xlsx"

    # 若是 Drive 檔案連結（file/d/<ID>），也嘗試轉直連下載
    m = re.search(r"/file/d/([^/]+)/", u)
    if m:
        file_id = m.group(1)
        return f"https://drive.google.com/uc?export=download&id={file_id}"

    # 其他格式：原樣回傳（讓 requests 自己 redirect）
    return u

def ensure_excel_from_gdrive(xlsx_path: Path) -> bool:
    """嘗試從 Google Drive / Sheets 下載 Excel；失敗就不阻斷流程（仍可用既有檔案或上傳）。"""
    url = st.secrets.get("GOOGLE_DRIVE_XLSX_URL", "") or DEFAULT_GDRIVE_URL
    url = _to_gsheets_export_xlsx_url(url)
    ...
    try:
        ...
        # xlsx 是 zip，檔頭通常是 PK；避免抓到 Google 的 HTML 預覽頁
...
st.sidebar.title("設定")
st.sidebar.button("重新載入資料（Google Drive）", on_click=_touch_reload_flag)
st.sidebar.caption(f"Google Drive：{'已設定' if st.secrets.get('GOOGLE_DRIVE_XLSX_URL') else '使用預設連結'}")
